app.controller("CompanyDetailController", [ '$scope', '$state', '$rootScope', '$filter', '$stateParams', 'Utils', 'APPServices', function($scope, $state, $rootScope, $filter, $stateParams, Utils, APPServices) {
	...
	$scope.activateCompany = function() {
		$state.go('home.admin.company.detail.activate', { record_id: $scope.data.id });
	};
	...
	$scope.getRoleById = function() {
		...
		APPServices.getCompanyById({ id: $stateParams.record_id }).then(function(response) {
			$scope.data = response.data;
			normalizeData();
		});
	};
