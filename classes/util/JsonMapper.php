...
class JsonMapper
{
     ...
     * @return mixed Mapped $array is returned
     */
    public function mapObject($json, $array = null)
    {
    	if (empty($json) || $this->isFlatType(gettype($json))) return $json;
    	...
    	foreach ($json as $key => $jvalue) {
    		...
    		if ($this->isFlatType($type)) {
    			...
    			$newArray = array(); $number = false;
    			foreach ($jvalue as $ikey => $ivalue) {
    				if (gettype($ikey) == 'integer' || gettype($ikey) == 'int') $number = true;
    				$newArray[$ikey] = $this->mapObject($ivalue);
    			}
    			if (!$number) {
    				$array[$key] = (object) $newArray;
    			} else {
    				$array[$key] = $newArray;
    			}
    		} else {
    			$array[$key] = $this->mapObject($jvalue);
    		}
    	}
    	return (object) $array;
    }
